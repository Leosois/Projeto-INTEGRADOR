<?php
include "header.php";
?>



<section id="principal">

    <div class="container">

        <div class="conteudo">

            <div class="titulo">

                <h2> Intel Core i9 10400F</h2>
                
            </div>

            <div class="img">
                <img class="itemIMG" src="src/img/img8.jpg" alt="">
            </div>

           

        </div>




        <div class="painel">

            <div class="desc">
                <h2>Processador</h2>
                <p>Intel i9 10400F</p>
                <h2>Placa de Video</h2>
                <p>Nvidia Geforce Rtx 1050</p>
                <h2>Memória</h2>
                <p>DDR4 32GB</p>
                <h2>Armazenamento</h2>
                <p>SSD 1TB</p>
            </div>

            <div class="desc">
                <div class="preco-original">de R$ 5.401,20 por</div>

                <div class="preco-pix">R$ 2.700,60</div>

                <div class="desconto-info">na troca do seu usado 50% de desconto</div>

                <div class="ou">ou</div>

                <div class="preco-parcelado-container">
                    <div class="preco-parcelado">R$ 5.999,65</div>
                    <div class="parcelas-info">
                        em até 12x de <span class="parcela-valor">R$ 499,97</span><br>
                        sem juros no cartão
                    </div>
                </div>

                

                <form action="adicionar_carrinho.php" method="POST">
                <input type="hidden" name="produto_id" value="07">
                <input type="hidden" name="nome" value="Intel Core i9 10400F">
                <input type="hidden" name="preco" value="5401.20">

                <button type="submit" class="pagamento-button comprar-button">
                    <div style="display: flex; align-items: center;">
                    <h4 class="comprar1">Comprar</h4>
                    </div>
                    
                </button>
            </form>
            </div>



        </div>
</section>

</div>



<?php
include "footer.php";
?>